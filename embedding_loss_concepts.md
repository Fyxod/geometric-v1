...
This phase does not hook Flux transformer hidden states. It treats Flux and the configured diffusion model as a black box and compares accessible inputs, outputs, embeddings, and latents.
...
## Black-Box With Respect To Flux

The optimizer does not backpropagate through Flux. It does not read Flux transformer hidden states in this phase.

Instead, each candidate follows this expensive but stable loop:
...
- VAE and CLIP backend status under `embedding_backends`
